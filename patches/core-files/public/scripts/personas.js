...
async function createDummyPersona() {
    ...
    const avatarId = `${Date.now()}-${personaName.replace(/[^a-zA-Z0-9]/g, '')}.jpg`;
...
export async function convertCharacterToPersona(characterId = null) {
    ...
    const overwriteName = `${name} (Persona).jpg`;
...
async function duplicatePersona(avatarId, { silent = false, select = false } = {}) {
    ...
    const newAvatarId = `${Date.now()}-${personaName.replace(/[^a-zA-Z0-9]/g, '')}.jpg`;
...
async function createPersonaCallback(args) {
    ...
    const avatarId = `${Date.now()}-${trimmedName.replace(/[^a-zA-Z0-9]/g, '')}.jpg`;
